...
def apply_pt_sl_on_t1(close, events, pt_sl, molecule):  # pragma: no cover
    ...
    """
    out = pd.DataFrame(index=molecule, columns=['pt', 'sl'])
    for t0 in molecule:
        t1 = events.at[t0, 't1'] if 't1' in events.columns else close.index[-1]
        path = close.loc[t0:t1]
        if len(path) == 0:
            continue
        ret = (path / close.loc[t0] - 1.0) * (events.at[t0, 'side'] if 'side' in events.columns else 1.0)
        pt = pt_sl[0] * events.at[t0, 'trgt'] if pt_sl[0] > 0 else np.nan
        sl = -pt_sl[1] * events.at[t0, 'trgt'] if pt_sl[1] > 0 else np.nan
        if pt_sl[0] > 0:
            pt_hit = ret[ret > pt].index.min()
            out.at[t0, 'pt'] = pt_hit
        if pt_sl[1] > 0:
            sl_hit = ret[ret < sl].index.min()
            out.at[t0, 'sl'] = sl_hit
    return out
...
def add_vertical_barrier(t_events, close, num_days=0, num_hours=0, num_minutes=0, num_seconds=0):
    ...
    """
    delta = pd.Timedelta(days=num_days, hours=num_hours, minutes=num_minutes, seconds=num_seconds)
    t1 = {}
    for t in t_events:
        dt = t + delta
...
def get_events(close, t_events, pt_sl, target, min_ret, num_threads, vertical_barrier_times=False,
    ...
    if vertical_barrier_times is False:
        ...
        t1 = vertical_barrier_times.reindex(trgt.index)
    events = pd.DataFrame({'t1': t1, 'trgt': trgt})
    if side_prediction is not None:
        events['side'] = side_prediction.reindex(events.index)
    events['pt'] = pt_sl[0]
    events['sl'] = pt_sl[1]
...
def barrier_touched(out_df, events):
    ...
    """
    out = out_df.copy()
    pt = events['pt'] * events['trgt']
    sl = -events['sl'] * events['trgt']
    out['bin'] = 0
    out.loc[out['ret'] > pt, 'bin'] = 1
    out.loc[out['ret'] < sl, 'bin'] = -1
    return out
...
def get_bins(triple_barrier_events, close):
    ...
    px = close.reindex(events.index).values
    px1 = close.reindex(events['t1']).values
    ret = px1 / px - 1.0
