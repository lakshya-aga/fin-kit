...
class StandardBars(BaseBars):
    ...
    def _extract_bars(self, data: Union[list, tuple, np.ndarray]) -> list:
        ...
        for date_time, price, volume in data:
            ...
            if isinstance(self.threshold, pd.Series):
                if date_time in self.threshold.index:
                    thr = self.threshold.loc[:date_time].iloc[-1]
                else:
                    thr = self.threshold.iloc[-1] if self.threshold.index.max() < date_time else self.threshold.iloc[0]
            else:
                thr = self.threshold
            metric_value = self.cum_dollar_value if "dollar" in self.metric else self.cum_volume if "volume" in self.metric else self.cum_ticks
            if metric_value >= thr:
...
def get_dollar_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], threshold: Union[float, pd.Series] = 70000000,
    ...
    """
    bars = StandardBars(metric="dollar", threshold=threshold, batch_size=batch_size)
...
def get_volume_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], threshold: Union[float, pd.Series] = 70000000,
    ...
    """
    bars = StandardBars(metric="volume", threshold=threshold, batch_size=batch_size)
    return bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
...
def get_tick_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], threshold: Union[float, pd.Series] = 70000000,
    ...
    :return: (pd.DataFrame) Dataframe of volume bars
    """
    bars = StandardBars(metric="tick", threshold=threshold, batch_size=batch_size)
    return bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
